...
type OrderOnWhatsAppButtonProps = {
  productName: string;
  price: string | number;
  quantity?: number;
  className?: string;
  /** When true (default), prevents parent card/link clicks from firing. */
...
export function OrderOnWhatsAppButton({
  productName,
  price,
  quantity = 1,
  className = '',
  ...
  onClick,
}: OrderOnWhatsAppButtonProps) {
  const [phone, setPhone] = useState(getDefaultWhatsappPhone);
  ...
  useEffect(() => {
    ...
    siteConfigApi
      ...
      .then((res) => {
        if (cancelled) return;
        const apiPhone = normalizeWhatsappNumber(res.data.whatsappNumber);
        const resolved = apiPhone || normalizeWhatsappNumber(getDefaultWhatsappPhone());
        if (resolved) setPhone(resolved);
      })
      .catch(() => {
        // Keep env-based phone from initial state.
      });
  ...
  const href = buildProductOrderWhatsappUrl({
    phone,
    productName,
    price,
    quantity,
